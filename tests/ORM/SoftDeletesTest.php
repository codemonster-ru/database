...
class SoftDeletesTest extends TestCase
{
    ...
    public function test_query_scopes(): void
    {
        ...
        $this->assertInstanceOf(SoftDeletingUser::class, $without[0]);
        $this->assertEquals('Active', $without[0]->name);

        $this->assertCount(1, $only);
        $this->assertInstanceOf(SoftDeletingUser::class, $only[0]);
        $this->assertEquals('Deleted', $only[0]->name);

        $this->assertCount(2, $with);
    }

    public function test_delete_returns_false_for_unsaved_model(): void
    {
    ...
    public function test_soft_delete_and_restore_events_are_fired(): void
    {
        $events = [];
        ...
        $deleted = SoftDeletingUser::find(1);
        $restored = SoftDeletingUser::find(2);

        $this->assertInstanceOf(SoftDeletingUser::class, $deleted);
        $this->assertInstanceOf(SoftDeletingUser::class, $restored);
    ...
    public function test_restore_event_can_cancel_restore(): void
    {
